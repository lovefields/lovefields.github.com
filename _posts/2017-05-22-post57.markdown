...
<div>문자열을 저장하기 위해서 문자형 배열 변수를 인스턴스 변수 value가 정의되어 있다.</div>
{% highlight java %}
...
<div><strong>String(String s)</strong></div>
<div>주어진 문자열(s)을 갖는 String 인스턴스를 생성.</div>
{% highlight java %}
...
<div><strong>String(StringBuffer buf)</strong></div>
...
<div><strong>int compareTo(String str)</strong></div>
<div>문자열(str)과 사전순서로 비교한다. 사전 순으로 이전이면 음수, 같으면 0, 이후면 양수를 반환한다.</div>
{% highlight java %}
...
<div><strong>boolean equals(Object obj)</strong></div>
<div>매개변수로 받은 문자열(obj)과 Stirng 인스턴스의 문자열을 비교한다.</div>
...
<div><strong>boolean equalsIgnoreCase(String str)</strong></div>
...
<div><strong>int indexOf(int ch, int pos)</strong></div>
<div>주어진 문자(ch)가 문자열에 존재하는지 지정된 위치(pos)부터 확인하여 위치(index)를 알려준다. 못 찾으면 -1을 반환한다.</div>
{% highlight java %}
String str = "Love";
...
<div><strong>String replace(char old, char nw)</strong></div>
<div>문자열 중의 문자(old)를 새로운 문자(nw)로 바꾼 문자열을 반환한다.</div>
{% highlight java %}
String s1 = "love";
...
<div><strong>String replace(CharSequence old, CharSequence nw)</strong></div>
<div>문자열 중의 문자열(old)를 새로운 문자열(nw)로 모두 바꾼 문자열을 반환한다.</div>
{% highlight java %}
String s1 = "lovelove";
...
<div><strong>String[] split(String regex, String replacement)</strong></div>
...
<div><strong>boolean startsWith(prefix)</strong></div>
...
<div><strong>String substring(int begin)</strong></div>
<div><strong>substring(int begin, int end)</strong></div>
<div>주어진 시작위치(begin)부터 끝 위치(end) 범위에 포함된 문자열을 얻는다.</div>
...
<div><strong>static String valueOf(boolean b)</strong></div>
<div><strong>static String valueOf(char c)</strong></div>
<div><strong>static String valueOf(int i)</strong></div>
<div><strong>static String valueOf(long l)</strong></div>
<div><strong>static String valueOf(float f)</strong></div>
<div><strong>static String valueOf(double d)</strong></div>
<div><strong>static String valueOf(Object o)</strong></div>
