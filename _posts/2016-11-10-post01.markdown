...
"Unastrongle to download data from https:rustrongygemsorg - SSL_connect returned=1 errno=0 state=SSLv3 read server certificate strong: certificate verify failed"

이 에러 뜨시는 분은 아래 게시물에서 "For windows users" 부분 참고해서 rustrongy 설치 후 jekyll 설치 진행하시면 됩니다!<br>
